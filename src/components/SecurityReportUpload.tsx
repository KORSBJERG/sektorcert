...
import { Upload, FileText, Loader2, CheckCircle2, AlertCircle, FileUp } from "lucide-react";
...
export function SecurityReportUpload({ 
  ...
  const handlePdfSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Create security report record (without AI analysis)
      const { data: report, error: reportError } = await supabase
        .from("security_reports")
        .insert({
          customer_id: customerId,
  ...
  const getStatusText = (type: "csv" | "pdf") => {
    switch (status) {
      case "uploading":
        return "Uploader fil...";
      case "analyzing":
      ...
      case "complete":
        return type === "csv" ? "Analyse færdig!" : "Upload færdig!";
      case "error":
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => {
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v as "csv" | "pdf"); resetState(); }}>
