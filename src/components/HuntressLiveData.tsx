...
export const HuntressLiveData = ({ customerId, huntressOrganizationId }: Props) => {
  ...
  const agentsRow = latest("agents");
  const incidentsRow = latest("incidents");

  const agents = ((agentsRow?.data as any)?.items ?? []) as any[];
  const incidents = ((incidentsRow?.data as any)?.items ?? []) as any[];
